---
title: 01. Rational and Markdown basics
...
So a piece of software like [pandoc](https://pandoc.org/) can read in a Markdown file and translate the formatting to html, thus giving you a new file you can put up as a webpage (Figure \@ref(fig:pandoc-img)).
This is part of the reason why plaintext formats are so versatile.

(ref:pandoc-img) An illustration of pandoc conversions

```{r pandoc-img, fig.cap='(ref:pandoc-img)', echo=FALSE, out.width = "60%"}
knitr::include_graphics("pandoc.png")
```

# What does R Markdown add?

All R Markdown adds is an extra layer prior to producing outputs.
Before pandoc is used, `knitr` runs through the document looking for any R code to evaluate (Figure \@ref(fig:rmarkdown-workflow))
...
(ref:rmarkdown-workflow) An illustration of the R Markdown compilation

```{r rmarkdown-workflow, fig.cap='(ref:rmarkdown-workflow)', echo=FALSE, out.width = "60%"}
knitr::include_graphics("rmarkdown_workflow.png")
```
