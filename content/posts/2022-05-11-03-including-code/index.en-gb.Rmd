...
```{r hold-position, fig.cap='Two plots side-by-side.', fig.show='hold', out.width='25%', eval=FALSE}
par(mar = c(4, 4, .2, .1))
plot(cars, pch = 19)
...
````markdown
`r ''````{r, out.width='25%', fig.align='center', fig.cap='The R Markdown hex logo.'}
...
```{r include-graphics, echo=FALSE, fig.cap='The R Markdown hex logo.', out.width='25%', fig.align='center', eval=FALSE}
## knitr::include_graphics('../../../../../img/rmarkdown_logo.jpg')
...
````markdown
(ref:external-caption) I'm outside the chunk! I can have **formatting** and *stuff*.

`r ''````{r, external-caption, fig.cap= '(\ref:external-caption)'}
plot(cars, pch = 18)
```
````
